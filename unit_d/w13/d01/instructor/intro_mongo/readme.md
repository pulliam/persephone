...
Take to the internet! Take 10 minutes to research MongoDB online and try to answer the following questions:

  - What is MongoDB?
  - What is NoSQL? How is it different from SQL?
  - Why would I use a NoSQL database?
  - How is data stored in a MongoDB database? What's the format? Is there a schema?
  - How can I access and update data using MongoDB?
...
Lets take a look at what a MongoDB _document_ may look like:
...
1. MongoDB by default will look for data in a folder named `/data/db`. We would have had to create this folder, but Homebrew did it for us (hopefully).
   1. run this command in your terminal (note - you may have to run this command twice):
```shell
...
## Updating Data - Codealong (10 mins)
...
## Cheese 1 Exercise (15 mins)

[Cheesercising](`./cheese_1.md`)
...
- ***After 5 mins, draw it out, then model it on screen.***
- ***Practice using the `$push`, `$pull`, and `$pop` operators.***

## Cheeses 2 Exercise

[Cheesercise 2](./cheese_2.md)

[Update Operators](https://docs.mongodb.org/v3.0/reference/operator/update/)
...
- What are the commands used to perform basic CRUD operations in MongoDB? What are the corresponding SQL commands?
